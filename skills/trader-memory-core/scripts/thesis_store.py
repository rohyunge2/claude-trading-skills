...
def _finalize_futures_outcome(
    ...
    *,
    ...
    into a LOCAL variable and validated BEFORE any mutation of
    thesis/position/status_history. Validation has two layers:

      1. `try/except OverflowError` around both the arithmetic and its
         `math.isfinite()` check (Issue #258). A huge Python int loaded
         from a hand-edited thesis can raise during multiplication,
         division, ledger summation, or `math.isfinite()` itself.
      2. The existing `math.isfinite()` result check for arithmetic that
         succeeds but produces inf/nan from individually finite operands.

    Neither failure mode may ever persist into thesis state.
    """
    entry_price = thesis["entry"].get("actual_price")
    entry_date = thesis["entry"].get("actual_date")
    position = thesis["position"]
...
def _trim_futures(
    ...
    # can still overflow their PRODUCT to inf. Issue #258 adds the
    # complementary arithmetic guard for a huge int loaded from disk:
    # multiplication or math.isfinite() can raise OverflowError before a
    # boolean finite result exists. Both are validated before mutation.
    try:
